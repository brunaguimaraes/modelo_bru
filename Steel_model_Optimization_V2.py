...
FALTA:
    ...
    FEITO: --Fazer a lista: Rota-> MM -> infos (faço isso com o transpose)
...
steel_production['Share_BOF_EAF'] = steel_production['EAF']/steel_production['Total']
...
for route in pd.unique(mitigation_measures.Rota):
    mitigation_dict = {}
    mitigation = {}
    for etapa in pd.unique(mitigation_measures.loc[mitigation_measures['Rota']==route,'Etapa']):
#        mitigation=mitigation_measures.loc[mitigation_measures['Rota']==route].loc[mitigation_measures['Etapa']==etapa].set_index('Medida de mitigacao').drop(['Rota','Etapa'],axis = 1).transpose().to_dict()
#        mitigation_dict[etapa] = mitigation
        mitigation=mitigation_measures.loc[mitigation_measures['Rota']==route].set_index('Medida de mitigacao').drop(['Rota'],axis = 1).transpose().to_dict()
    mitigation_measures_dict[route] = mitigation
    
# teste = mitigation_measures.loc[mitigation_measures['Rota']==route].set_index('Medida de mitigacao').drop(['Rota'],axis = 1).transpose().to_dict()
...
for rota in pd.unique(EI_BEU['Rota']):
    a_dict={}
    for etapa in pd.unique(EI_BEU.loc[EI_BEU['Rota']==rota]['Etapa']):
        a =  EI_BEU.loc[EI_BEU['Rota']==rota].loc[EI_BEU['Etapa']==etapa].set_index('Combustivel').drop(['Rota','Etapa'],axis =1)
        a = a.to_dict()
        a_dict[etapa] = a
    EI_dict[rota] = a_dict   
#%%
...
steel_production['Share_BOF_EAF'][2050] = steel_production['Share_BOF_EAF'][base_year]
...
def optimization_module(year,emission):
    ...
    model.X1 =  Var (k1,within =NonNegativeReals) #Energy efficiency mitigation measurex
    model.X2 =  Var (k2,within =NonNegativeReals) #Energy efficiency mitigation measurex
    model.X3 =  Var (k3,within =NonNegativeReals) #Energy efficiency mitigation measurex
    model.X4 =  Var (k4,within =NonNegativeReals) #Energy efficiency mitigation measurex
    model.X5= Var(within = NonNegativeReals) #DR-NG share
    model.X6 = Var(within = NonNegativeReals) #Charcoal
    model.X7 = Var(within = NonNegativeReals) #DR-H2
#    
    #creating objective
    ...
    #Capex of each route
    capex_R1 = sum((mitigation_measures_dict['R1'][i]['CAPEX (RS/t)'])*model.X1[i]*(float(steel_production.loc[year]['Share_BOF_MC'])-model.X5-model.X6-model.X7)*steel_production.loc[year]['Total']/1000 for i in k1)
    capex_R2 = sum((mitigation_measures_dict['R2'][i]['CAPEX (RS/t)'])*model.X2[i]*steel_production.loc[year]['EAF']/1000 for i in k2)
    capex_R3 = sum((mitigation_measures_dict['R3'][i]['CAPEX (RS/t)'])*model.X3[i]*steel_production.loc[year]['BOF CC']/1000 for i in k3)
    capex_R4 = sum((mitigation_measures_dict['R4'][i]['CAPEX (RS/t)'])*model.X4[i]*pig_iron_production.loc[year]['Independente CV']/1000 for i in k4)
    ...
    opex_R1 = sum((mitigation_measures_dict['R1'][i]['OPEX (RS/t)'])*model.X1[i]*(float(steel_production.loc[year]['Share_BOF_MC'])-model.X5-model.X6-model.X7)*steel_production.loc[year]['Total']/1000 for i in k1)
    opex_R2 = sum((mitigation_measures_dict['R2'][i]['OPEX (RS/t)'])*model.X2[i]*steel_production.loc[year]['EAF']/1000 for i in k2)
    opex_R3 = sum((mitigation_measures_dict['R3'][i]['OPEX (RS/t)'])*model.X3[i]*steel_production.loc[year]['BOF CC']/1000 for i in k3)
    opex_R4 = sum((mitigation_measures_dict['R4'][i]['OPEX (RS/t)'])*model.X4[i]*pig_iron_production.loc[year]['Independente CV']/1000 for i in k4)
    

    Emission_mitigated_R1 = sum(model.X1[i]*mitigation_measures_dict['R1'][i]['Reducao na intensidade (Gj/t)']*sum(EI_dict['R1'][mitigation_measures_dict['R1'][i]['Etapa']]['2020'][x]/sum(EI_dict['R1'][mitigation_measures_dict['R1'][i]['Etapa']]['2020'].values())*emission_factor.loc[x]['CO2'] for x in EI_dict['R1'][mitigation_measures_dict['R1'][i]['Etapa']]['2020']) for i in k1)*(float(steel_production.loc[year]['Share_BOF_MC'])-model.X5-model.X6-model.X7)*steel_production.loc[year]['Total']/10**6
    Emission_mitigated_R2 = sum(model.X2[i]*mitigation_measures_dict['R2'][i]['Reducao na intensidade (Gj/t)']*sum(EI_dict['R2'][mitigation_measures_dict['R2'][i]['Etapa']]['2020'][x]/sum(EI_dict['R2'][mitigation_measures_dict['R2'][i]['Etapa']]['2020'].values())*emission_factor.loc[x]['CO2'] for x in EI_dict['R2'][mitigation_measures_dict['R2'][i]['Etapa']]['2020']) for i in k2)*steel_production.loc[year]['Total']*(model.X6 + steel_production.loc[year]['Share_BOF_CC'])/10**6
    Emission_mitigated_R3 = sum(model.X3[i]*mitigation_measures_dict['R3'][i]['Reducao na intensidade (Gj/t)']*sum(EI_dict['R3'][mitigation_measures_dict['R3'][i]['Etapa']]['2020'][x]/sum(EI_dict['R3'][mitigation_measures_dict['R3'][i]['Etapa']]['2020'].values())*emission_factor.loc[x]['CO2'] for x in EI_dict['R3'][mitigation_measures_dict['R3'][i]['Etapa']]['2020']) for i in k3)*steel_production.loc[year]['EAF']/10**6
    Emission_mitigated_R4 = sum(model.X4[i]*mitigation_measures_dict['R4'][i]['Reducao na intensidade (Gj/t)']*sum(EI_dict['R4'][mitigation_measures_dict['R4'][i]['Etapa']]['2020'][x]/sum(EI_dict['R4'][mitigation_measures_dict['R4'][i]['Etapa']]['2020'].values())*emission_factor.loc[x]['CO2'] for x in EI_dict['R4'][mitigation_measures_dict['R4'][i]['Etapa']]['2020']) for i in k4)*pig_iron_production.loc[year]['Independente CV']/10**6
    ...
    #Energy consumption by route without mitigation measures:
    EC_R1_no_measure = (float(R1_EI_Total[year]))*(float(steel_production.loc[year]['Share_BOF_MC'])-model.X5-model.X6-model.X7)*steel_production.loc[year]['Total']
    EC_R2_no_measure=  (float(R2_EI_Total[year]))*steel_production.loc[year]['Total']*(model.X6+steel_production.loc[year]['Share_BOF_CC'])
    EC_R3_no_measure=+(float(R3_EI_Total[year]))*steel_production.loc[year]['EAF']
    EC_R4_no_measure= +(float(R4_EI_Total[year]))*pig_iron_production.loc[year]['Independente CV']
    
    Emission_Baseline = (
            EC_R1_no_measure*sum(Energy_share_R1.loc[f][year]*emission_factor.loc[f]['CO2'] for f in Energy_share_R1.index)/10**6
            +EC_R2_no_measure * sum(Energy_share_R2.loc[f][year]*emission_factor.loc[f]['CO2'] for f in Energy_share_R2.index)/10**6
            +EC_R3_no_measure *sum(Energy_share_R3.loc[f][year]*emission_factor.loc[f]['CO2'] for f in Energy_share_R3.index)/10**6
            +EC_R4_no_measure*sum(Energy_share_R4.loc[f][year]*emission_factor.loc[f]['CO2'] for f in Energy_share_R4.index)/10**6
            )
...
##    Fuel cost when applying mitigation measures: REFAZER
    Energy_saving_R1 = sum(model.X1[i]*mitigation_measures_dict['R1'][i]['Reducao na intensidade (Gj/t)']*sum(EI_dict['R1'][mitigation_measures_dict['R1'][i]['Etapa']]['2020'][x]/sum(EI_dict['R1'][mitigation_measures_dict['R1'][i]['Etapa']]['2020'].values())*fuel_prices.loc[x]['BRL/TJ'] for x in EI_dict['R1'][mitigation_measures_dict['R1'][i]['Etapa']]['2020']) for i in k1)*(float(steel_production.loc[year]['Share_BOF_MC'])-model.X5-model.X6-model.X7)*steel_production.loc[year]['Total']/10**6
    Energy_saving_R2 = sum(model.X2[i]*mitigation_measures_dict['R2'][i]['Reducao na intensidade (Gj/t)']*sum(EI_dict['R2'][mitigation_measures_dict['R2'][i]['Etapa']]['2020'][x]/sum(EI_dict['R2'][mitigation_measures_dict['R2'][i]['Etapa']]['2020'].values())*fuel_prices.loc[x]['BRL/TJ'] for x in EI_dict['R2'][mitigation_measures_dict['R2'][i]['Etapa']]['2020']) for i in k2)*steel_production.loc[year]['Total']*(model.X6 + steel_production.loc[year]['Share_BOF_CC'])/10**6
    Energy_saving_R3 = sum(model.X3[i]*mitigation_measures_dict['R3'][i]['Reducao na intensidade (Gj/t)']*sum(EI_dict['R3'][mitigation_measures_dict['R3'][i]['Etapa']]['2020'][x]/sum(EI_dict['R3'][mitigation_measures_dict['R3'][i]['Etapa']]['2020'].values())*fuel_prices.loc[x]['BRL/TJ'] for x in EI_dict['R3'][mitigation_measures_dict['R3'][i]['Etapa']]['2020']) for i in k3)*steel_production.loc[year]['EAF']/10**6
    Energy_saving_R4 = sum(model.X4[i]*mitigation_measures_dict['R4'][i]['Reducao na intensidade (Gj/t)']*sum(EI_dict['R4'][mitigation_measures_dict['R4'][i]['Etapa']]['2020'][x]/sum(EI_dict['R4'][mitigation_measures_dict['R4'][i]['Etapa']]['2020'].values())*fuel_prices.loc[x]['BRL/TJ'] for x in EI_dict['R4'][mitigation_measures_dict['R4'][i]['Etapa']]['2020']) for i in k4)*pig_iron_production.loc[year]['Independente CV']/10**6

    Energy_cost_R5_R6 = +(model.X5*steel_production.loc[year]['Total']*innovation_measures.loc[0]['Energy_intensity (GJ/t)'] + model.X7*steel_production.loc[year]['Total']*innovation_measures.loc[2]['Energy_intensity (GJ/t)'])*fuel_prices.loc['Gas natural']['BRL/TJ']/10**6
    +(model.X5*steel_production.loc[year]['Total']*innovation_measures.loc[1]['Energy_intensity (GJ/t)'] + model.X7*steel_production.loc[year]['Total']*innovation_measures.loc[3]['Energy_intensity (GJ/t)'])*fuel_prices.loc['Eletricidade']['BRL/TJ']/10**6
   
  
#    Fuel economy: Fuel cost A - Fuel cost saving
    fuel_saving = Energy_saving_R1+ Energy_saving_R2 + Energy_saving_R3+ Energy_saving_R4 - Energy_cost_R5_R6
    
    model.obj= Objective(expr =(capex_R1+ capex_R2+capex_R3 + capex_R4 + capex_R5 + model.X6*1+ capex_R6)*interest_rate*((1+interest_rate)**20)/((1+interest_rate)**20-1)
                         +opex_R1+opex_R2+opex_R3+opex_R4
                         -fuel_saving #Falta arrumar o custo do vegetal
    )
    ...
    ##Penetration
    for i in k1:
        model.con.add(model.X1[i] <= float(mitigation_measures_dict['R1'][i]['Penetracao']))
    for i in k2:
        model.con.add (model.X2[i]<=float(mitigation_measures_dict['R2'][i]['Penetracao']))
    for i in k3:
        model.con.add (model.X3[i]<=float(mitigation_measures_dict['R3'][i]['Penetracao']))
    for i in k4:
        model.con.add (model.X4[i]<=float(mitigation_measures_dict['R4'][i]['Penetracao']))
    ...
    model.con.add(model.X5+model.X6+model.X7 <= float(steel_production.loc[year]['Share_BOF_MC']))
...
#exemplo de como posso fazer o calculo das emissoes
    def emission_calc_route (route):
        ...
        for step in EI_dict[route].keys():
            emission = emission+sum(EI_dict[route][step]['2020'][f]*emission_factor.loc[f]['CO2'] for f in EI_dict[route][step]['2020'].keys())
        return emission
...
#Emission_R1 = sum(EI_dict['R1']['Alto-forno']['2020'][f]*emission_factor.loc[f]['CO2'] for f in EI_dict['R1']['Alto-forno']['2020'].keys())
...
#Tem que colocar o 5 e o 6 como redução das emissões.
    model.con.add(
           ...
           + Emission_mitigated_R3
            ...
            +EC_R5_calc*emission_factor.loc['Gas natural']['CO2']/10**6
            +EC_R6_calc*emission_factor.loc['Gas natural']['CO2']/10**6
            <=emission
            )
...
Results = pd.DataFrame(columns = [2030,2040,2050],index = ['Cost','Emissions','Energy Consumption','Fuel_saving'],data=0,dtype=float)
...
#
cost_curve = pd.DataFrame(columns = ['Cost'],index = np.linspace(0.99,0.30,100),data = 0, dtype=float)
emission_mitigated = pd.DataFrame(columns = ['Emission mitigated'], index = np.linspace(0.99,0.30,100), data = 0, dtype = float)

for i in np.linspace(0.99,0.4,100):
...
for step in EI_dict['R1'].keys():
    for measure in mitigation_measures_dict['R1']:
        if  step == mitigation_measures_dict['R1'][measure]['Etapa']:
            print(measure,mitigation_measures_dict['R1'][measure]['Etapa'],step == mitigation_measures_dict['R1'][measure]['Etapa'])
        else:
            pass
        
#for i in k1:
#    for x in EI_dict['R1'][mitigation_measures_dict['R1'][i]['Etapa']]['2020']:
#        EI_dict['R1'][mitigation_measures_dict['R1'][i]['Etapa']]['2020'][x] *=1-model.X1()*mitigation_measures_dict['R1'][i]['Reducao na intensidade (Gj/t)']
