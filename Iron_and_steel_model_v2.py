...
for year in np.linspace(2020,2050,2050-2020+1).astype(int):
...
for indice in Cost_saved_charcoal_shift.index:
    Cost_saved_charcoal_shift.loc[indice,'2005':] = Energy_mitigated_charcoal_shift.loc[indice, '2005':]*float(fuel_prices.loc[fuel_prices['Combustivel']==Cost_saved_charcoal_shift.loc[indice]['Combustivel']]['BRL/TJ'])/(10**6)


Cost_saved_EAF_shift = Energy_mitigated_EAF_shift.copy()
for indice in Cost_saved_EAF_shift.index:
    Cost_saved_EAF_shift.loc[indice,'2005':] = Energy_mitigated_EAF_shift.loc[indice, '2005':]*float(fuel_prices.loc[fuel_prices['Combustivel']==Cost_saved_EAF_shift.loc[indice]['Combustivel']]['BRL/TJ'])/(10**6)
...
for indice in Cost_saved_mitigation_measures.index:
    Cost_saved_mitigation_measures.loc[indice,'2005':] = Energy_mitigated_by_measures.loc[indice, '2005':]*float(fuel_prices.loc[fuel_prices['Combustivel']==Cost_saved_mitigation_measures.loc[indice]['Combustivel']]['BRL/TJ'])/(10**6)
...
Emissions_mitigated_by_measure_CO2 = pd.DataFrame(columns =Emissions_mitigated_by_R1_CO2.columns, dtype=object)
...
Emissions_mitigated_by_measure_CH4 = pd.DataFrame(columns =Emissions_mitigated_by_R1_CH4.columns,dtype=object)
...
Emissions_mitigated_by_measure_N2O = pd.DataFrame(columns =Emissions_mitigated_by_R1_N2O.columns,dtype=object)
for medida in mitigation_measures['Medida de mitigacao']:
    if mitigation_measures.loc[mitigation_measures['Medida de mitigacao'] == medida]['Rota'].any() == 'R1':
...
for indice in mitigation_measures.index:
    if indice == 0:
        capex = (mitigation_measures.loc[indice,'CAPEX (RS/t)']*1000*steel_production.loc[2020:,route_name[mitigation_measures.loc[indice,'Rota']]]*mitigation_measures_penetration.iloc[indice,3:]).tolist()
        capex = np.array([capex])
    else:
        if mitigation_measures.loc[indice,'Rota'] == 'R4':
            x = (mitigation_measures.loc[indice,'CAPEX (RS/t)']*1000*pig_iron_production.loc[2020:,route_name[mitigation_measures.loc[indice,'Rota']]]*mitigation_measures_penetration.iloc[indice,3:]).tolist()
            x= np.array([x])
            capex = np.append(capex,x,axis=0)
        else:
            x = (mitigation_measures.loc[indice,'CAPEX (RS/t)']*1000*steel_production.loc[2020:,route_name[mitigation_measures.loc[indice,'Rota']]]*mitigation_measures_penetration.iloc[indice,3:]).tolist()
            x= np.array([x])
            capex = np.append(capex,x,axis = 0)
...
for indice in mitigation_measures.index:
    if indice == 0:
        opex = (mitigation_measures.loc[indice,'OPEX (RS/t)']*1000*steel_production.loc[2020:,route_name[mitigation_measures.loc[indice,'Rota']]]*mitigation_measures_penetration.iloc[indice,3:]).tolist()
        opex = np.array([opex])
    else:
        if mitigation_measures.loc[indice,'Rota'] == 'R4':
            x = (mitigation_measures.loc[indice,'OPEX (RS/t)']*1000*pig_iron_production.loc[2020:,route_name[mitigation_measures.loc[indice,'Rota']]]*mitigation_measures_penetration.iloc[indice,3:]).tolist()
            x= np.array([x])
            opex = np.append(opex,x,axis=0)
        else:
            x = (mitigation_measures.loc[indice,'OPEX (RS/t)']*1000*steel_production.loc[2020:,route_name[mitigation_measures.loc[indice,'Rota']]]*mitigation_measures_penetration.iloc[indice,3:]).tolist()
            x= np.array([x])
            opex = np.append(opex,x,axis = 0)
