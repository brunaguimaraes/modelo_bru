...
current_year = 2019
# Horizon decenal:
future_years = list(range(current_year+1, 2051))  # 2019 a 2050 - future_years é simplesmente uma lista Python dos anos seguintes em que seu modelo vai instalar plantas, produzir e mitigar emissões.
routes = ['R1', 'R2', 'R3', 'R4']
...
"""Importing Energy Consumption compatible with the Useful Energy Balance (BEU):
    ...
    """
EI_BEU = pd.read_csv('C:/Users/Bruna/OneDrive/DOUTORADO/0.TESE/modelagem/modelo_bru/teste/EI_Route_Step_year_teste3.csv', sep=';') #minha intensidade energetica por rota e combustivel, sem considerar as etapas
...
# Anos do histórico
past_years = np.linspace(2005, 2019, 2019-2005+1, dtype=int)
...
Energy_consumption_BEN = pd.read_excel("C:/Users/Bruna/OneDrive/DOUTORADO/0.TESE/modelagem/modelo_bru/CE_Siderurgia_novo.xlsx") #importing BEN_Steel
...
anos_validos = [ano for ano in past_years if ano in R1_EI_Total.index and ano in steel_production['BOF MC'].index]

# essa parte aqui dos anos é importante!
...
Energy_share_R1 = Total_energy_consumption_R1.set_index('Combustivel').drop(columns=['routes'])
Energy_share_R2 = Total_energy_consumption_R2.set_index('Combustivel').drop(columns=['routes'])
Energy_share_R3 = Total_energy_consumption_R3.set_index('Combustivel').drop(columns=['routes'])
Energy_share_R4 = Total_energy_consumption_R4.set_index('Combustivel').drop(columns=['routes'])
