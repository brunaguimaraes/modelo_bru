...
Cheguei a conclusão que não quero parar em 2019, como eu estava prevendo. Quero fazer como o otto de ir até 2024. preciso 
rever isso ao longo de todo o modelo e tambem dos dados históricos.
...
for Rota in pd.unique(EI_BEU['Rota']):
    a_dict={}
    for etapa in pd.unique(EI_BEU.loc[EI_BEU['Rota']==Rota]['Step']):
        a =  EI_BEU.loc[EI_BEU['Rota']==Rota].loc[EI_BEU['Step']==etapa].set_index('Combustivel').drop(['Rota','Step'],axis =1)
        a = a.to_dict()
        a_dict[etapa] = a
    EI_dict[Rota] = a_dict   
#%%
