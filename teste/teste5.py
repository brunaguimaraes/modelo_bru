...
for indice in mitigation_measures.index:
    ...
    """    
       # Calcula o somatório total de redução por Route
total_reduction_by_route = (
    mitigation_measures.groupby('Route')['Total Reduction GJ/t'].transform('sum')
)
...
def optimization_module(year, emission_target, capacity_exist, cum_new_cap):
      model = ConcreteModel()  
...
"""agora falando sobre medidas de mit"""
...
model.X4 =  Var (k4,within =NonNegativeReals) #Energy efficiency mitigation measure in Route 4
   ...
   """
    ...
    #Capex of each route
    capex_R1 = sum((mitigation_measures_dict['R1'][i]['CAPEX ($/t)']) * model.X1[i] * production_R1 / 1000 for i in k1) + 170 * model.newCap['R1']  #170 é o valor do capex
    capex_R2 = sum((mitigation_measures_dict['R2'][i]['CAPEX ($/t)']) * model.X2[i] * production_R2 / 1000 for i in k2) + 170 * model.newCap['R2']
    capex_R3 = sum((mitigation_measures_dict['R3'][i]['CAPEX ($/t)']) * model.X3[i] * production_R3 / 1000 for i in k3) + 184 * model.newCap['R3']
    capex_R4 = sum((mitigation_measures_dict['R4'][i]['CAPEX ($/t)'])*model.X4[i]*production_R4/1000 for i in k4)
    capex_R5 = model.prod_R5 * innovation_measures.loc[0]['CAPEX (Euro/t)'] / 1000
    capex_R6 = model.prod_R6 * innovation_measures.loc[2]['CAPEX (Euro/t)'] / 1000
    capex_R7 = model.prod_R7 * innovation_measures.loc[4]['CAPEX (Euro/t)'] / 1000
    capex_CCS = model.prod_CCS * innovation_measures.loc[6]['CAPEX (Euro/t)'] / 1000
 
    
    #Capex of each route ANTIGO
   ...
   # capex_R4 = sum((mitigation_measures_dict['R4'][i]['CAPEX ($/t)'])*model.X4[i]*production_R4/1000 for i in k4)
    ...
    opex_R1 = sum(mitigation_measures_dict['R1'][i]['OPEX ($/t)'] * model.X1[i] * production_R1 / 1000 for i in k1) + opex_base['R1'] * model.newCap['R1']
    opex_R2 = sum(mitigation_measures_dict['R2'][i]['OPEX ($/t)'] * model.X2[i] * production_R2 / 1000 for i in k2) + opex_base['R2'] * model.newCap['R2']
    opex_R3 = sum(mitigation_measures_dict['R3'][i]['OPEX ($/t)'] * model.X3[i] * production_R3 / 1000 for i in k3) + opex_base['R3'] * model.newCap['R3']
    opex_R4 = sum(mitigation_measures_dict['R4'][i]['OPEX ($/t)'] * model.X4[i] * production_R4 / 1000 for i in k4) + opex_base['R4'] * model.newCap['R4']
    ...
    opex_R5 = (model.prod_R5 *innovation_measures.loc[0]['OPEX']/1000)
    opex_R6 = (model.prod_R6 *innovation_measures.loc[2]['OPEX']/1000)
    opex_R7 = (model.prod_R7 *innovation_measures.loc[4]['OPEX']/1000)
    opex_CCS = (model.prod_CCS *innovation_measures.loc[6]['OPEX']/1000)
    
#Emission mitigated considering energy efficiency measures and fuel shift


"""aqui tem etapa!!!!!!!!!!!!!!!!!!!!!!!! terei que rever"""
"""['Step']]['2023'][x]/sum(EI_d"""


    Emission_mitigated_R1 = sum(model.X1[i]*mitigation_measures_dict['R1'][i]['Energy reduction (Gj/t)']*sum(EI_dict['R1'][mitigation_measures_dict['R1'][i]]['2023'][x]/sum(EI_dict['R1'][mitigation_measures_dict['R1'][i]]['2023'].values())*emission_factor.loc[x]['CO2e'] for x in EI_dict['R1'][mitigation_measures_dict['R1'][i]]['2023']) for i in k1)*production_R1/10**6
    Emission_mitigated_R2 = sum(model.X2[i]*mitigation_measures_dict['R2'][i]['Energy reduction (Gj/t)']*sum(EI_dict['R2'][mitigation_measures_dict['R2'][i]]['2023'][x]/sum(EI_dict['R2'][mitigation_measures_dict['R2'][i]]['2023'].values())*emission_factor.loc[x]['CO2e'] for x in EI_dict['R2'][mitigation_measures_dict['R2'][i]]['2023']) for i in k2)*production_R2/10**6
    Emission_mitigated_R3 = sum(model.X3[i]*mitigation_measures_dict['R3'][i]['Energy reduction (Gj/t)']*sum(EI_dict['R3'][mitigation_measures_dict['R3'][i]]['2023'][x]/sum(EI_dict['R3'][mitigation_measures_dict['R3'][i]]['2023'].values())*emission_factor.loc[x]['CO2e'] for x in EI_dict['R3'][mitigation_measures_dict['R3'][i]]['2023']) for i in k3)*production_R3/10**6
    Emission_mitigated_R4 = sum(model.X4[i]*mitigation_measures_dict['R4'][i]['Energy reduction (Gj/t)']*sum(EI_dict['R4'][mitigation_measures_dict['R4'][i]]['2023'][x]/sum(EI_dict['R4'][mitigation_measures_dict['R4'][i]]['2023'].values())*emission_factor.loc[x]['CO2e'] for x in EI_dict['R4'][mitigation_measures_dict['R4'][i]]['2023']) for i in k4)*production_R4/10**6

  #ANTIGO
  #aqui tem etapa!!!!!!!!!!!!!!!!!!!!!!!! EXCLUÍ STEP
...
##    Fuel cost when applying mitigation measures: REFAZER
    ...
    Energy_saving_R1 = sum(model.X1[i]*mitigation_measures_dict['R1'][i]['Energy reduction (Gj/t)']*sum(EI_dict['R1'][mitigation_measures_dict['R1'][i]]['2023'][x]/sum(EI_dict['R1'][mitigation_measures_dict['R1'][i]]['2023'].values())*fuel_prices.loc[x]['BRL/TJ'] for x in EI_dict['R1'][mitigation_measures_dict['R1'][i]]['2023']) for i in k1)*production_R1/10**6
    Energy_saving_R2 = sum(model.X2[i]*mitigation_measures_dict['R2'][i]['Energy reduction (Gj/t)']*sum(EI_dict['R2'][mitigation_measures_dict['R2'][i]]['2023'][x]/sum(EI_dict['R2'][mitigation_measures_dict['R2'][i]]['2023'].values())*fuel_prices.loc[x]['BRL/TJ'] for x in EI_dict['R2'][mitigation_measures_dict['R2'][i]]['2023']) for i in k2)*production_R2/10**6
    Energy_saving_R3 = sum(model.X3[i]*mitigation_measures_dict['R3'][i]['Energy reduction (Gj/t)']*sum(EI_dict['R3'][mitigation_measures_dict['R3'][i]]['2023'][x]/sum(EI_dict['R3'][mitigation_measures_dict['R3'][i]]['2023'].values())*fuel_prices.loc[x]['BRL/TJ'] for x in EI_dict['R3'][mitigation_measures_dict['R3'][i]]['2023']) for i in k3)*production_R3/10**6
    Energy_saving_R4 = sum(model.X4[i]*mitigation_measures_dict['R4'][i]['Energy reduction (Gj/t)']*sum(EI_dict['R4'][mitigation_measures_dict['R4'][i]]['2023'][x]/sum(EI_dict['R4'][mitigation_measures_dict['R4'][i]]['2023'].values())*fuel_prices.loc[x]['BRL/TJ'] for x in EI_dict['R4'][mitigation_measures_dict['R4'][i]]['2023']) for i in k4)*production_R4/10**6
...
+((EC_R1_no_measure*Energy_share_R1.loc['Carvao vegetal'][year]
    ...
    def emission_calc_route (route):
        ...
        for step in EI_dict[route].keys():
            emission = emission+sum(EI_dict[route]['2023'][f]*emission_factor.loc[f]['CO2e'] for f in EI_dict[route]['2023'].keys())
        return emission
...
def optimization_module(year, emission_target, capacity_exist, cum_new):
model = ConcreteModel()  
...
model.con = ConstraintList()
model.con.add(production_R1 <= capacity_exist['R1'][year] + cum_new['R1'] + model.newCap['R1'])
model.con.add(production_R2 <= capacity_exist['R2'][year] + cum_new['R2'] + model.newCap['R2'])
model.con.add(production_R3 <= capacity_exist['R3'][year] + cum_new['R3'] + model.newCap['R3'])
model.con.add(production_R4 <= capacity_exist['R4'][year] + cum_new['R4'] + model.newCap['R4'])
