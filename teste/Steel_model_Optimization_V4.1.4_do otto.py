...
#
caminho = 'C:/Users/ottoh/OneDrive/Doutorado/Tese/Resultados/Imagine/MIT1/MIT1_V9/'
# caminho = 'C:/Users/Bruna/OneDrive/Tese/Resultados/Imagine/MIT1/MIT1_V9/'
cenario = 'MIT1_V9'#
#%%
"""1. Importing Data"""
...
EI_BEU['2024']=EI_BEU['2023'] #adicionando o ano de 2024
...
mitigation_measures = pd.read_excel(
    caminho+"medidas de mitigação unificadas.xlsx",
    sheet_name=SECTOR
)
...
# fuel_prices = pd.read_csv("https://raw.githubusercontent.com/ottohebeda/Industry_Energy_Emissions_simulator/main/fuel_prices_V9.xlsx")

#Importing fuel prices
fuel_prices = pd.read_excel('https://raw.githubusercontent.com/ottohebeda/Industry_Energy_Emissions_simulator/main/fuel_prices_V9.xlsx')
fuel_prices = fuel_prices.rename(columns={'Fuel':'Combustivel'})
fuel_prices = fuel_prices.rename(index={'Pneus':'Residuos'})
#fuel_prices['BRL/TJ']= fuel_prices['BRL/ktep']/ktoe_to_tj
fuel_prices['US$/TJ']= fuel_prices['US$/ktoe']/ktoe_to_tj
fuel_prices['BRL/TJ']= fuel_prices['US$/TJ']/dolar
fuel_prices = fuel_prices.rename(index={'Produtos da cana':'Bagaco de cana'})
fuel_prices = fuel_prices.rename(index={'Residuos agricolas':'Lixivia'})
...
past_years = np.linspace(2005,2024,2024-2005+1,dtype = int)

#Future years:
future_years = np.linspace(2025,2050,2050-2025+1,dtype = int)
...
Energy_consumption_BEN = pd.read_excel("CE_Siderurgia 04-07-25.xlsx") #importing BEN_Steel
...
for year in np.linspace(2025,2050,2050-2025+1).astype(int):
...
Production_increase = pd.read_excel(caminho+'Nivel_producao_'+cenario+'.xlsx')
Production_increase =Production_increase.set_index('Sector')
Production_increase = Production_increase.loc['IronSteel']
Production_increase =Production_increase.to_dict()
...
#    Emission mitigated considering energy efficiency measures and fuel shift
    Emission_mitigated_R1 = sum(model.X1[i]*mitigation_measures_dict['R1'][i]['Energy reduction (Gj/t)']*sum(EI_dict['R1'][mitigation_measures_dict['R1'][i]['Step']]['2024'][x]/sum(EI_dict['R1'][mitigation_measures_dict['R1'][i]['Step']]['2024'].values())*emission_factor.loc[x]['CO2e'] for x in EI_dict['R1'][mitigation_measures_dict['R1'][i]['Step']]['2024']) for i in k1)*production_R1/10**6
    Emission_mitigated_R2 = sum(model.X2[i]*mitigation_measures_dict['R2'][i]['Energy reduction (Gj/t)']*sum(EI_dict['R2'][mitigation_measures_dict['R2'][i]['Step']]['2024'][x]/sum(EI_dict['R2'][mitigation_measures_dict['R2'][i]['Step']]['2024'].values())*emission_factor.loc[x]['CO2e'] for x in EI_dict['R2'][mitigation_measures_dict['R2'][i]['Step']]['2024']) for i in k2)*production_R2/10**6
    Emission_mitigated_R3 = sum(model.X3[i]*mitigation_measures_dict['R3'][i]['Energy reduction (Gj/t)']*sum(EI_dict['R3'][mitigation_measures_dict['R3'][i]['Step']]['2024'][x]/sum(EI_dict['R3'][mitigation_measures_dict['R3'][i]['Step']]['2024'].values())*emission_factor.loc[x]['CO2e'] for x in EI_dict['R3'][mitigation_measures_dict['R3'][i]['Step']]['2024']) for i in k3)*production_R3/10**6
    Emission_mitigated_R4 = sum(model.X4[i]*mitigation_measures_dict['R4'][i]['Energy reduction (Gj/t)']*sum(EI_dict['R4'][mitigation_measures_dict['R4'][i]['Step']]['2024'][x]/sum(EI_dict['R4'][mitigation_measures_dict['R4'][i]['Step']]['2024'].values())*emission_factor.loc[x]['CO2e'] for x in EI_dict['R4'][mitigation_measures_dict['R4'][i]['Step']]['2024']) for i in k4)*production_R4/10**6
...
#    capex_total = (capex_R1+ capex_R2+capex_R3 + capex_R4 + capex_R5+ capex_R6+capex_R7+capex_CCS)*interest_rate*((1+interest_rate)**20)/((1+interest_rate)**20-1)
    ...
    # model.con.add(model.X5<=0)
    model.con.add(model.X8<=0.0) #SR
    # model.con.add(model.X9<=0.0) #EAF
    # model.con.add(model.X6<=0.0) #EAF
    # model.con.add(model.X7<=0)
    # model.con.add(model.X7<=0)
...
+((EC_R1_no_measure*Energy_share_R1.loc['Carvao vegetal'][year]
        ...
        -(sum(model.X1[i]*mitigation_measures_dict['R1'][i]['Energy reduction (Gj/t)']*EI_dict['R1'][mitigation_measures_dict['R1'][i]['Step']]['2024']['Carvao vegetal']/sum(EI_dict['R1'][mitigation_measures_dict['R1'][i]['Step']]['2024'].values()) for i in k1)*production_R1
    +sum(model.X2[i]*mitigation_measures_dict['R2'][i]['Energy reduction (Gj/t)']*EI_dict['R2'][mitigation_measures_dict['R2'][i]['Step']]['2024']['Carvao vegetal']/sum(EI_dict['R2'][mitigation_measures_dict['R2'][i]['Step']]['2024'].values()) for i in k2)*production_R2
     +sum(model.X3[i]*mitigation_measures_dict['R3'][i]['Energy reduction (Gj/t)']*EI_dict['R3'][mitigation_measures_dict['R3'][i]['Step']]['2024']['Carvao vegetal']/sum(EI_dict['R3'][mitigation_measures_dict['R3'][i]['Step']]['2024'].values()) for i in k3)*production_R3
    +sum(model.X4[i]*mitigation_measures_dict['R4'][i]['Energy reduction (Gj/t)']*EI_dict['R4'][mitigation_measures_dict['R4'][i]['Step']]['2024']['Carvao vegetal']/sum(EI_dict['R4'][mitigation_measures_dict['R4'][i]['Step']]['2024'].values()) for i in k4)*production_R4
)
    )
        <= 576812*0.8)
        # <= 692750)
    ...
    def emission_calc_route (route):
        ...
        for step in EI_dict[route].keys():
            emission = emission+sum(EI_dict[route][step]['2024'][f]*emission_factor.loc[f]['CO2e'] for f in EI_dict[route][step]['2024'].keys())
        return emission
    ...
    #Emission_R1 = sum(EI_dict['R1']['Alto-forno']['2024'][f]*emission_factor.loc[f]['CO2e'] for f in EI_dict['R1']['Alto-forno']['2024'].keys())
    ...
    for fuel in CE.index:
        CE[fuel] = (CE[fuel]
        -sum(model.X1[i]()*mitigation_measures_dict['R1'][i]['Energy reduction (Gj/t)']*EI_dict['R1'][mitigation_measures_dict['R1'][i]['Step']]['2024'][fuel]/sum(EI_dict['R1'][mitigation_measures_dict['R1'][i]['Step']]['2024'].values()) for i in k1)*production_R1()
        -sum(model.X2[i]()*mitigation_measures_dict['R2'][i]['Energy reduction (Gj/t)']*EI_dict['R2'][mitigation_measures_dict['R2'][i]['Step']]['2024'][fuel]/sum(EI_dict['R2'][mitigation_measures_dict['R2'][i]['Step']]['2024'].values())for i in k2)*production_R2()
        -sum(model.X3[i]()*mitigation_measures_dict['R3'][i]['Energy reduction (Gj/t)']*EI_dict['R3'][mitigation_measures_dict['R3'][i]['Step']]['2024'][fuel]/sum(EI_dict['R3'][mitigation_measures_dict['R3'][i]['Step']]['2024'].values()) for i in k3)*production_R3()
        -sum(model.X4[i]()*mitigation_measures_dict['R4'][i]['Energy reduction (Gj/t)']*EI_dict['R4'][mitigation_measures_dict['R4'][i]['Step']]['2024'][fuel]/sum(EI_dict['R4'][mitigation_measures_dict['R4'][i]['Step']]['2024'].values()) for i in k4)*production_R4
        )
#    
...
anos = list(range(2025, 2051))

Emission_reduction = pd.read_excel(
    caminho+"Nivel_Emissao"+"_"+cenario+'.xlsx'
)
    
Emission_reduction = Emission_reduction.set_index("Sector")
Emission_reduction = Emission_reduction.loc['IronSteel']
Emission_reduction = Emission_reduction.interpolate(method ='index')
Emission_reduction = Emission_reduction.to_dict()

Emission_reduction[2023] =1

Emission_base = pd.DataFrame(data = np.linspace(1,1,len(Emission_reduction)),index= Emission_reduction)
Emission_base = Emission_base[0].to_dict()

Results = pd.DataFrame(columns = Emission_reduction,index = [
...
X1 = pd.DataFrame(columns = Emission_reduction, index =  mitigation_measures_dict['R1'].keys(),data= 0,dtype=float)
X2 = pd.DataFrame(columns = Emission_reduction, index =  mitigation_measures_dict['R2'].keys(),data= 0,dtype=float)
CE_mit = pd.DataFrame(columns = Emission_reduction,index =Energy_share_R1.index,data=0, dtype=float )
CE_ref =  pd.DataFrame(columns = Emission_reduction,index =Energy_share_R1.index,data=0, dtype=float )
...
mitigacao_df = pd.DataFrame(0, index=Emission_reduction, columns=colunas)
...
for year in Energy_consumption_BEN.columns[10:]:
...
emissoes_gas_processo = pd.DataFrame(data = 0, index = ['CO2','CH4', 'N2O'], columns = list(range(2023, 2051)))
emissoes_gas_energia = pd.DataFrame(data = 0, index = ['CO2','CH4', 'N2O'], columns = list(range(2023, 2051)))

for year in list(range(2023, 2051)):
...
excel_file = 'Energia_Imagine_'+cenario+'.xlsx'
...
"""Custos"""
# Define the Excel file name
excel_file = 'Custos_Imagine_'+cenario+'.xlsx'
...
"""Custos de mitigacao"""
# Define the Excel file name
excel_file = 'MAC_Imagine_'+cenario+'.xlsx'
...
"""Emissões"""
# Define the Excel file name
excel_file = 'EmissoesEnergia_Imagine_'+cenario+'.xlsx'
...
excel_file = 'EmissoesProcesso_Imagine_'+cenario+'.xlsx'
file_path = output_directory + excel_file
