...
for year in [2023, 2030, 2040, 2050]:
    if year not in emission_limit_dict:
        print(f"  {year}: No emission constraint (historical/base year)")
        continue
...
# Actual emissions from optimizer
plt.plot(emissions_df["Year"], emissions_df["Emissions"], 
         'b-', linewidth=2, label='Model Emissions')

# Linear reference path (base_year → 2050)
ref_years = [y for y in model_years if y >= base_year]
ref_values = [linear_reference(y) for y in ref_years]
plt.plot(ref_years, ref_values, 
         'r--', linewidth=2, label='Linear Target Path')

# Flexibility band (10% above reference, only intermediate years)
band_years = [y for y in model_years if base_year < y < year_end]
band_values = [linear_reference(y) * (1.0 + flexibility) for y in band_years]
if band_years:
    plt.plot(band_years, band_values, 
             'r:', linewidth=1, alpha=0.5, label=f'+{flexibility:.0%} Flexibility Band')
    # Shade the corridor between reference and band
    plt.fill_between(band_years, 
                     [linear_reference(y) for y in band_years], 
                     band_values, 
                     color='red', alpha=0.08)

# Mark key points
plt.scatter([base_year], [linear_reference(base_year)], 
            color='red', zorder=5, s=50)
plt.scatter([year_end], [emission_2050], 
...
output_path = r"C:\Users\Bruna\OneDrive\DOUTORADO\0.TESE\modelagem\modelo_bru\teste\resultados\resultados_modelo_V18.xlsx"
