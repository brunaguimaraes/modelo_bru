...
# NAS SEÇOES 10 E 12 QUE EU ACHO QUE EU CRIAVA PLANTAS VIRTUAIS ADICIONAIS PRA SURPIR TODA A DEMANDA HISTÓRICA.Deixei eles comentados. Se for precisarei deletar depois do script!
...
# CHANGED: base_production comes from plants_unified (endogenous),
# not from steel_production (national historical data).
# This ensures the model's production trajectory is consistent
# with the actual plants in the model.

base_production = plants_unified['Production_2023_final'].sum()

# Create clean DataFrame for production targets (model years only)
steel_total_target = pd.DataFrame(
    index=pd.Index(model_years, name='Year'),
    columns=['Total'],
    dtype=float
)

# Set base year
steel_total_target.loc[2023, 'Total'] = base_production
...
print("\n=== PRODUCTION TARGETS ===")
print(f"Base production (from plants): {base_production:,.0f} kt")
...
# Linear interpolation of emission limits from 2020 to 2050
emission_limit_dict = {
...
# --- HELPER: Endogenous total production ---
# Used by share constraints (D, I, J, K, L) so that shares are always
# relative to what the model actually produces, not to an external target.
# This makes the constraints self-consistent regardless of production level.
def total_production_expr(m, year):
    """Sum of all plant production in a given year (endogenous)"""
...
# CHANGED: shares are relative to endogenous total production,
# not to the external ProductionTarget parameter.
def penetration_rule_production(m, tech, year):
    """Production by technology limited by penetration rate of total production"""
    ...
    return (
        ...
        <= penetration_lim * total_production_expr(m, year)
    )
...
def share_balance_rule(m, year):
    ...
    CHANGED: uses endogenous total production instead of ProductionTarget
    Total innovative production <= Share_BOF_MC * total_production
    """
    ...
    return innovative_production <= BASE_SHARE_BOF_MC * total_production_expr(m, year)
...
def charcoal_route_share_rule(m, year):
    ...
    return total_bfbof_cc <= CHARCOAL_ROUTE_SHARE_LIMIT * total_production_expr(m, year)
...
def sr_share_rule(m, year):
    ...
    return total_sr <= SR_SHARE_LIMIT * total_production_expr(m, year)
...
#
# CHANGED: uses endogenous total production instead of ProductionTarget.
# This ensures the share constraint is always internally consistent,
# regardless of the absolute production level.
...
def mc_share_cap_rule(m, year):
    ...
    return total_mc <= BASE_SHARE_MC * total_production_expr(m, year)
...
print(f"    (uses endogenous total production, not external target)")
...
# IMPORTANT: Model is now NONLINEAR because share constraints (D, I, J, K, L)
# use total_production_expr (sum of variables) on the right-hand side.
# This means production_tech <= share × sum(all_production), which is
# a product of variables. IPOPT handles this; HiGHS cannot.
solver_name = "ipopt"
...
output_path = r"C:\Users\Bruna\OneDrive\DOUTORADO\0.TESE\modelagem\modelo_bru\teste\resultados\resultados_modelo_V19.xlsx"
