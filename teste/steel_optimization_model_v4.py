# -*- coding: utf-8 -*-
"""
Steel Production Optimization Model - Version 3
...
Updates in V3:
- Added fuel cost module
- Integrated energy intensity by route and fuel
- Fuel prices integrated into objective function
"""
...
print("\n=== TECHNOLOGY PARAMETERS CHECK (including fuel costs) ===")
for route in routes_in_plants:
    if route in measures_dict:
        ...
        print(f"  {route}: CAPEX={capex}, OPEX={opex}, Emission={emiss}, Fuel={fuel:.2f} USD/t")
    else:
...
def obj_rule(m):
    ...
    - CAPEX: Charged in the start year, levelized over plant lifetime
    - OPEX: Charged every year based on production
    - FUEL COSTS: Charged every year based on production
    
    All costs are in consistent units (USD or BRL)
    """
    total = 0
    ...
    for plant in m.Plant:
        ...
        if tech in measures_dict:
            capex = float(measures_dict[tech].get('CAPEX', 0) or 0)  # USD/t capacity
            opex = float(measures_dict[tech].get('OPEX', 0) or 0)    # USD/t produced
            fuel_cost = float(measures_dict[tech].get('Fuel_cost_USD_t', 0) or 0)  # USD/t produced
        else:
            capex = 0
            opex = 0
            fuel_cost = 0
        ...
        for year in m.Year:
            # CAPEX charged in the start year (levelized)
            if int(plant_attr[plant]['Startyear']) == int(year):
                total += capex * plant_attr[plant]['Capacity'] * LEVELIZED_FACTOR
...
# Energy consumption = Production (kt) * Energy Intensity (GJ/t) * 1000 (t/kt)
energy_consumption = []
...
for year in m.Year:
    for route in routes_in_plants:
        ...
        # Get energy intensity for this route
        if route in EI_by_route:
            ei = EI_by_route[route]
        else:
            # Use estimated values for innovative routes
            ei_estimates = {
                'DR-NG': 19.9,  # GJ/t
                'DR-H2': 20.2,
                'SR': 19.4,
        ...
        # Energy consumption in TJ (Production in kt * EI in GJ/t = 1000 * GJ = TJ)
        ec_tj = prod * ei  # Actually this is in 1000 GJ = TJ since prod is in kt
...
energy_by_route_wide = energy_df.pivot_table(
    index='Route',
    columns='Year',
    values='Energy_Consumption_TJ',
    aggfunc='sum'
).reset_index()

# Total energy consumption by year
total_energy_by_year = energy_df.groupby('Year')['Energy_Consumption_TJ'].sum().reset_index()
total_energy_by_year.columns = ['Year', 'Total_Energy_TJ']

print("\nTotal Energy Consumption (selected years, TJ):")
for year in [2023, 2030, 2040, 2050]:
    if year in total_energy_by_year['Year'].values:
        val = total_energy_by_year[total_energy_by_year['Year'] == year]['Total_Energy_TJ'].values[0]
        print(f"  {year}: {val:,.0f} TJ")
...
plt.figure(figsize=(12, 6))
energy_pivot = energy_df.pivot_table(index='Year', columns='Route', values='Energy_Consumption_TJ', aggfunc='sum')
...
output_path = r"C:\Users\Bruna\OneDrive\DOUTORADO\0.TESE\modelagem\modelo_bru\teste\resultados\resultados_modelo_V15.xlsx"
...
with pd.ExcelWriter(output_path) as writer:
    ...
    # --- ENERGY RESULTS ---
    energy_by_route_wide.to_excel(writer, sheet_name="Energy_by_Route", index=False)
    total_energy_by_year.to_excel(writer, sheet_name="Total_Energy", index=False)
...
print("  5. Cost_Breakdown          - CAPEX, OPEX, Fuel costs by year")
print("  6. Cost_by_Year            - Costs in wide format")
print("  7. Energy_by_Route         - Energy consumption by route")
print("  8. Total_Energy            - Total energy consumption by year")
print("  9. Production_Detail_Long  - Long format for analysis")
print(" 10. Total_Production        - Total production by year")
print(" 11. Fuel_Cost_Parameters    - Fuel costs by route")
print(" 12. Summary                 - Model parameters")
print(" 13. Input_Penetration_Limits")
print(" 14. Input_Technology_Params")
