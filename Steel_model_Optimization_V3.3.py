...
12/09 - Custo de combustível tava errado (acho qeu tava em dolar). Agora ajustei aqui e tá entrando muito ccs...preciso rever isso.
"""
...
#    capex_total = (capex_R1+ capex_R2+capex_R3 + capex_R4 + capex_R5+ capex_R6+capex_R7+capex_CCS)*interest_rate*((1+interest_rate)**20)/((1+interest_rate)**20-1)
    ...
    model.con.add(model.X6+steel_production['Share_BOF_CC'][year]<=0.19) #charcoal limite
...
for i in Emission_reduction:
    x,capex_total,CE_1= optimization_module(i,emission_calc(i)*Emission_reduction[i])
