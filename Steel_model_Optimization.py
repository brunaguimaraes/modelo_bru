...
FALTA:
    -  Ajustar medidas de mitigação nas etapas corretas
...
for route in pd.unique(mitigation_measures.Rota):
    ...
    for etapa in pd.unique(mitigation_measures.loc[mitigation_measures['Rota']==route,'Etapa']):
        mitigation=mitigation_measures.loc[mitigation_measures['Rota']==route].set_index('Medida de mitigacao').drop(['Rota','Etapa'],axis = 1).transpose().to_dict()
        mitigation_dict[etapa] = mitigation
    mitigation_measures_dict[route] = mitigation_dict
...
def emission_calc (year):
    emission = (
        float(R1_EI_Total[year])*(float(steel_production.loc[year]['Share_BOF_MC']))*steel_production.loc[year]['Total']*sum(Energy_share_R1.loc[f][year]*emission_factor.loc[f]['CO2'] for f in Energy_share_R1.index)
        +(float(R2_EI_Total[year]))*steel_production.loc[year]['Total']*(steel_production.loc[year]['Share_BOF_CC'])*sum(Energy_share_R2.loc[f][year]*emission_factor.loc[f]['CO2'] for f in Energy_share_R2.index)
        +(float(R3_EI_Total[year]))*steel_production.loc[year]['EAF']*sum(Energy_share_R3.loc[f][year]*emission_factor.loc[f]['CO2'] for f in Energy_share_R3.index)
        +(float(R4_EI_Total[year]))*pig_iron_production.loc[year]['Independente CV']*sum(Energy_share_R4.loc[f][year]*emission_factor.loc[f]['CO2'] for f in Energy_share_R4.index)
        )/10**6
    return emission
...
def optimization_module(year,emission):
    ...
    #Creating variables
    k1 = np.linspace(0,mitigation_measures.Rota[mitigation_measures.Rota=='R1'].index[-1],mitigation_measures.Rota[mitigation_measures.Rota=='R1'].index[-1]+1,dtype=int) #list of the number of energy mitigation measures
    k2 = np.linspace(k1[-1]+1,mitigation_measures.Rota[mitigation_measures.Rota=='R2'].index[-1],mitigation_measures.Rota[mitigation_measures.Rota=='R2'].index[-1]-k1[-1],dtype=int) 
    k3 = np.linspace(k2[-1]+1,mitigation_measures.Rota[mitigation_measures.Rota=='R3'].index[-1],mitigation_measures.Rota[mitigation_measures.Rota=='R3'].index[-1]-k2[-1],dtype=int) 
    k4 =  np.linspace(k3[-1]+1,mitigation_measures.Rota[mitigation_measures.Rota=='R4'].index[-1],mitigation_measures.Rota[mitigation_measures.Rota=='R4'].index[-1]-k3[-1],dtype=int)
    ...
    #Capex of each route
    capex_R1 = sum((mitigation_measures.loc[i]['CAPEX (RS/t)']+mitigation_measures.loc[i]['OPEX (RS/t)'])*model.X1[i]*(float(steel_production.loc[year]['Share_BOF_MC'])-model.X5-model.X6-model.X7)*steel_production.loc[year]['Total']/1000 for i in k1)
    capex_R2 = sum((mitigation_measures.loc[i]['CAPEX (RS/t)']+mitigation_measures.loc[i]['OPEX (RS/t)'])*model.X2[i]*steel_production.loc[year]['EAF']/1000 for i in k2)
    capex_R3 = sum((mitigation_measures.loc[i]['CAPEX (RS/t)']+mitigation_measures.loc[i]['OPEX (RS/t)'])*model.X3[i]*steel_production.loc[year]['BOF CC']/1000 for i in k3)
    capex_R4 = sum((mitigation_measures.loc[i]['CAPEX (RS/t)']+mitigation_measures.loc[i]['OPEX (RS/t)'])*model.X4[i]*pig_iron_production.loc[year]['Independente CV']/1000 for i in k4)
    ...
    EC_R1_calc = (float(R1_EI_Total[year])-sum(model.X1[i]*float(mitigation_measures.loc[i]['Reducao na intensidade (Gj/t)']) for i in k1))*(float(steel_production.loc[year]['Share_BOF_MC'])-model.X5-model.X6-model.X7)*steel_production.loc[year]['Total']
    EC_R2_calc =  (float(R2_EI_Total[year])-sum(model.X2[i]*float(mitigation_measures.loc[i]['Reducao na intensidade (Gj/t)']) for i in k2))*steel_production.loc[year]['Total']*(model.X6 + steel_production.loc[year]['Share_BOF_CC'])
    EC_R3_calc =+(float(R3_EI_Total[year])-sum(model.X3[i]*float(mitigation_measures.loc[i]['Reducao na intensidade (Gj/t)']) for i in k3))*steel_production.loc[year]['EAF']
    EC_R4_calc = +(float(R4_EI_Total[year])-sum(model.X4[i]*float(mitigation_measures.loc[i]['Reducao na intensidade (Gj/t)']) for i in k4))*pig_iron_production.loc[year]['Independente CV']
    ...
    EC_R1_no_measure = (float(R1_EI_Total[year]))*(float(steel_production.loc[year]['Share_BOF_MC']))*steel_production.loc[year]['Total']
    EC_R2_no_measure=  (float(R2_EI_Total[year]))*steel_production.loc[year]['Total']*(steel_production.loc[year]['Share_BOF_CC'])
...
#    Fuel cost when applying mitigation measures:
    fuel_cost_mitigation = (
            EC_R1_calc*sum(Energy_share_R1.loc[f][year]*fuel_prices.loc[f]['BRL/TJ'] for f in Energy_share_R1.index)/10**6
            +EC_R2_calc*sum(Energy_share_R2.loc[f][year]*fuel_prices.loc[f]['BRL/TJ'] for f in Energy_share_R2.index)/10**6
            +EC_R3_calc*sum(Energy_share_R3.loc[f][year]*fuel_prices.loc[f]['BRL/TJ'] for f in Energy_share_R3.index)/10**6
            +EC_R4_calc*sum(Energy_share_R4.loc[f][year]*fuel_prices.loc[f]['BRL/TJ'] for f in Energy_share_R4.index)/10**6            
            +(model.X5*steel_production.loc[year]['Total']*innovation_measures.loc[0]['Energy_intensity (GJ/t)'] + model.X7*steel_production.loc[year]['Total']*innovation_measures.loc[2]['Energy_intensity (GJ/t)'])*fuel_prices.loc['Gas natural']['BRL/TJ']/10**6
...
#    Fuel economy: Fuel cost A - Fuel cost saving
    fuel_saving = fuel_cost_mitigation -fuel_cost_no_measure
    
    model.obj= Objective(expr =
     capex_R1 + capex_R2 + capex_R3 + capex_R4 + capex_R5 + model.X6*1+ capex_R6+fuel_saving #Falta arrumar o custo do vegetal
    )
    ...
    ##Penetration
    for i in k1:
        model.con.add(model.X1[i] <= float(mitigation_measures.loc[i]['Penetracao']))
    for i in k2:
        model.con.add (model.X2[i]<=float(mitigation_measures.loc[i]['Penetracao']))
    for i in k3:
        model.con.add (model.X3[i]<=float(mitigation_measures.loc[i]['Penetracao']))
    for i in k4:
        model.con.add (model.X4[i]<=float(mitigation_measures.loc[i]['Penetracao']))
    ...
    model.con.add(
            EC_R1_calc*sum(Energy_share_R1.loc[f][year]*emission_factor.loc[f]['CO2'] for f in Energy_share_R1.index)/10**6
            +EC_R2_calc * sum(Energy_share_R2.loc[f][year]*emission_factor.loc[f]['CO2'] for f in Energy_share_R2.index)/10**6
            +EC_R3_calc *sum(Energy_share_R3.loc[f][year]*emission_factor.loc[f]['CO2'] for f in Energy_share_R3.index)/10**6
            +EC_R4_calc*sum(Energy_share_R4.loc[f][year]*emission_factor.loc[f]['CO2'] for f in Energy_share_R4.index)/10**6
...
for i in Emission_reduction:
    x,y = optimization_module(i,emission_calc(i)*Emission_reduction[i])
...
#
#for i in np.linspace(0.99,0.30,100):
#    x,y= optimization_module(2020,emission_calc(2020)*i)
